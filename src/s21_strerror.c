...
const char *s21_maccos_errors_messages[ERR_COUNT] = {
    "Undefined error: 0",                               // 0
...
const char *s21_linux_errors_messages[ERR_COUNT] = {
    "Success",                                            // 0
    "Operation not permitted",                            // 1
    "No such file or directory",                          // 2
    "No such process",                                    // 3
...
char *s21_strerror(int errnum) {
  ...
  static char error_unknown[50];
  s21_strncpy(error_unknown, UNKNOWN_PREFIX, 50);

#if defined(__APPLE__) || defined(__MACH__)
  if (errnum >= 0 && errnum < ERR_COUNT) {
    error_text = (char *)s21_maccos_errors_messages[errnum];
  } else {
    s21_strncat(error_unknown, s21_convert_int_to_string(errnum), 50);
    error_text = error_unknown;
  }
#elif defined(__linux__)
  if (errnum >= 0 && errnum < ERR_COUNT) {
    error_text = (char *)s21_linux_errors_messages[errnum];
  } else {
    s21_strncat(error_unknown, s21_convert_int_to_string(errnum), 50);
    error_text = error_unknown;
  }
...
char *s21_convert_int_to_string(int num) {
  static char num_to_str[20];
  char num_to_str_temp[20];
  char part_of_num;
  int index = 0, minus = 0;
  ...
  while (num != 0) {
    part_of_num = num % 10 + '0';
    num_to_str_temp[index] = part_of_num;
    index++;
    num /= 10;
  }

  if (minus)
    num_to_str_temp[index] = '-';
  else
    index--;

  for (int i = index; i >= 0; i--) num_to_str[index - i] = num_to_str_temp[i];

  num_to_str[index + 1] = '\0';
